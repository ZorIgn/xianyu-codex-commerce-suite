...
class AutoDeliveryHandler:
    ...
    async def _handle_auto_delivery(self, websocket, message: dict, send_user_name: str, send_user_id: str,
        ...
        try:
            ...
            try:
                lock_result = await try_acquire_delivery_lock(order_id, expire=120, holder_info=self.cookie_id, wait_timeout=0.2)
                    ...
                    try:
                        ...
                        if fc_result.get('handled'):
                            if fc_result.get('success'):
                                ...
                                logger.info(
                                    f"[fulfillment-center] order shipped by local inventory: "
                                    f"order_id={order_id}, item_id={item_id}, quantity={quantity_to_send}"
                                )
                            else:
                                self._last_delivery_fail_reason = fc_result.get('message') or 'fulfillment center shipment failed'
                                logger.error(
                                    f"[fulfillment-center] order shipment blocked: "
                                    f"order_id={order_id}, item_id={item_id}, reason={self._last_delivery_fail_reason}"
                                )
                            quantity_to_send = 0
