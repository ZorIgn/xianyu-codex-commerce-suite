...
class XianyuAsync:
        ...
        try:
            ...
            if not hasattr(self, 'message_handler'):
                ...
                async def on_chat_message(parsed_message, ws):
                    ...
                    # 如果是付款相关消息，异步获取订单详情
                    await self._notify_fulfillment_center_buyer_message(parsed_message, ws)
                    self._create_tracked_task(self._process_order_status(raw_message, send_message, item_id, send_user_id, msg_time))
    ...
    def _compact_codex_message_text(self, value) -> str:
        text = str(value or "").lower()
        return "".join(ch for ch in text if ch.isalnum() or "\u4e00" <= ch <= "\u9fff")

    def _looks_like_codex_invite_confirmation(self, text: str) -> bool:
        normalized = self._compact_codex_message_text(text)
        return any(kw in normalized for kw in (
            "全部邮箱无误已邀请", "已邀请", "邀请了", "邀了", "发邀请", "发送邀请",
            "发送了", "发了", "弄好了", "好了", "ok", "okay", "完成了", "可以了"
        ))

    async def _find_latest_order_id_for_buyer(self, chat_id: str, buyer_id: str) -> str:
        try:
            from sqlalchemy import desc, or_, select
            from common.db.session import async_session_maker
            from common.models.xy_order import XYOrder

            filters = [XYOrder.account_id == self.cookie_id]
            matches = []
            if chat_id:
                matches.append(XYOrder.chat_id == str(chat_id))
            if buyer_id:
                matches.append(XYOrder.buyer_id == str(buyer_id))
            if not matches:
                return ""
            stmt = (
                select(XYOrder.order_no)
                .where(*filters, or_(*matches))
                .order_by(desc(XYOrder.updated_at))
                .limit(1)
            )
            async with async_session_maker() as session:
                result = await session.execute(stmt)
            ...
            logger.warning(f"[fulfillment-center] failed to lookup latest order for buyer message: {exc}")
            return ""

    async def _notify_fulfillment_center_buyer_message(self, parsed_message: dict, websocket) -> None:
        text = str(parsed_message.get("send_message") or "").strip()
        if not text or not self._looks_like_codex_invite_confirmation(text):
            return
        chat_id = str(parsed_message.get("chat_id") or "")
        buyer_id = str(parsed_message.get("send_user_id") or "")
        myid = str(parsed_message.get("myid") or "")
        if myid and buyer_id == myid:
            return
        raw_message = parsed_message.get("raw_message", {})
        order_id = self._extract_order_id(raw_message) if isinstance(raw_message, dict) else ""
        if not order_id:
            order_id = await self._find_latest_order_id_for_buyer(chat_id, buyer_id)
        if not order_id:
            logger.info(f"[fulfillment-center] invite-like message ignored because order_id is unknown: chat_id={chat_id}, buyer_id={buyer_id}")
            return

        try:
            from app.services.fulfillment_center_client import _base_url, _enabled
            import aiohttp
            if not _enabled():
                return
            payload = {"order_id": str(order_id), "buyer_id": buyer_id, "text": text}
            headers = {"Idempotency-Key": f"xianyu-message-{order_id}-{abs(hash(text))}"}
            timeout = aiohttp.ClientTimeout(total=20)
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.post(f"{_base_url()}/webhooks/xianyu/message", json=payload, headers=headers) as resp:
            ...
            logger.error(f"[fulfillment-center] buyer-message webhook exception: {exc}")
