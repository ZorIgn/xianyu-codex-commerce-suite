...
class AutoReplyService:
    ...
    def _prefix_auto_reply(self, text: str) -> str:
        """Add a visible marker to text generated by the automatic reply pipeline."""
        value = str(text or "")
        if not value or value.startswith("__IMAGE_SEND__"):
            return value
        if value.lstrip().startswith("[自动回复]"):
            return value
        return f"[自动回复] {value}"
    ...
    async def handle_chat_message(self, parsed_message: Dict[str, Any], websocket) -> None:
        ...
        try:
            ...
            if not should_skip:
                ...
                if reply:
                    if not reply.startswith("__IMAGE_SEND__"):
                        reply = self._prefix_auto_reply(reply)
                        log_payload["reply_text"] = reply
                        log_payload["reply_segments"] = self._build_text_reply_segments(reply)
                    ...
                    if reply.startswith("__IMAGE_SEND__"):
                        ...
                        if pending_text_reply:
                            pending_text_reply = self._prefix_auto_reply(pending_text_reply)
                            log_payload["reply_text"] = pending_text_reply
                            log_payload["reply_segments"] = self._build_text_reply_segments(pending_text_reply)
                            send_results.extend(
