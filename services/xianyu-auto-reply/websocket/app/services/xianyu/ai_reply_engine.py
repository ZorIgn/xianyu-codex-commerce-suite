...
CODEX_ACTIVITY_SYSTEM_PROMPT = """你是闲鱼 Codex 邀请活动客服，只能回答本商品相关问题。
商品说明：这是 Codex 邀请活动。买家拍下付款后，系统会发给买家邮箱；买家需要在 Codex 左下角找到 Invite Friends/邀请入口，把邮箱填进去并发送邀请。
固定要求：买家完成邀请后，必须在闲鱼回复一整句：全部邮箱无误 已邀请
硬性规则：
1. 不准砍价、不主动降价、不承诺优惠、不引导线下交易。
2. 不编造库存、额度、到账时间、官方规则、后台状态或任何未提供的信息。
3. 咨询阶段只说明活动是什么、拍下后发邮箱、如何邀请、固定回复词。
4. 买家说“已邀请”“邀请了”“发了”“好了”“OK”等类似表达时，必须回复：请直接回复：全部邮箱无误 已邀请
5. 回复必须中文、简短、像真人客服，不要说自己是 AI，不要输出分析过程。
"""
...
注意：结合商品信息，给出实用建议。"""
        ...
        self.default_prompts = {
            "price": CODEX_ACTIVITY_SYSTEM_PROMPT,
            "tech": CODEX_ACTIVITY_SYSTEM_PROMPT,
            "default": CODEX_ACTIVITY_SYSTEM_PROMPT,
        }
    ...
    def _compact_codex_text(self, value: Any) -> str:
        text = self._normalize_text(value).lower()
        return "".join(ch for ch in text if ch.isalnum() or "\u4e00" <= ch <= "\u9fff")

    def _rule_based_codex_reply(self, message: str, intent: str) -> Optional[str]:
        normalized = self._compact_codex_text(message)
        if not normalized:
            return None
        exact_trigger = "全部邮箱无误已邀请" in normalized
        invite_like = exact_trigger or any(kw in normalized for kw in (
            "已邀请", "邀请了", "邀了", "发邀请", "发送了", "发了", "弄好了", "好了", "ok", "okay", "完成了", "可以了"
        ))
        if exact_trigger:
            return "收到，正在为您激活，请稍等。"
        if invite_like:
            return "请直接回复：全部邮箱无误 已邀请"
        price_like = intent == "price" or any(kw in normalized for kw in (
            "便宜", "优惠", "砍价", "刀", "少点", "最低", "降价", "能少", "包邮"
        ))
        if price_like:
            return "这个是 Codex 邀请活动，价格按页面为准，不议价。拍下后会发邮箱给您。"
        consult_like = any(kw in normalized for kw in (
            "是什么", "怎么", "流程", "邀请", "codex", "邮箱", "使用", "教程"
        ))
        if consult_like:
            return "这是 Codex 邀请活动。拍下后发邮箱，您在 Codex 左下角邀请入口填写并发送。"
        return None
    ...
    async def generate_reply(
        ...
        try:
            ...
            async with chat_lock:
                ...
                rule_reply = self._rule_based_codex_reply(message, intent)
                if rule_reply:
                    rule_message = AIChatMessage(
                        chat_id=chat_id,
                        cookie_id=cookie_id,
                        user_id=user_id,
                        item_id=item_id,
