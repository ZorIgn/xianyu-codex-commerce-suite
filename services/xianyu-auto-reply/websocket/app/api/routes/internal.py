...
async def deliver_order(request: DeliverOrderRequest):
    ...
    try:
        ...
        # 开关：websocket/.env 设置 FULFILLMENT_CENTER_ENABLED=true。
        try:
            ...
            if fc_result.get("handled"):
                if not fc_result.get("success"):
                    return {
                        "success": False,
                        "code": 400,
                        "message": fc_result.get("message") or "履约中心出库失败",
                        "data": fc_result,
                    }
                delivery_text = fc_result["delivery_text"]
                ...
                if not send_ok:
                    return {
                        "success": False,
                        "code": 500,
                        "message": "履约中心已出库，但闲鱼消息发送失败，请到本地库存中心查看已出库记录后手动补发",
                        "data": fc_result,
                    }
                from common.services.order_service import OrderService
                async with async_session_maker() as session:
                    await OrderService(session).update_order_delivery_info(
                        order_no=request.order_no,
                        status="shipped",
                        delivery_method="fulfillment_center",
                        delivery_content=delivery_text,
                    )
                return {
                    "success": True,
                    "code": 200,
                    ...
                    "data": {
                        ...
                        "quantity": request.quantity,
                        "delivery_method": "fulfillment_center",
                        "fulfillment": fc_result.get("data"),
                    },
                }
        except Exception as fc_e:
            logger.error(f"【内部API】履约中心接管异常: {fc_e}")
            return {
                "success": False,
                "code": 500,
                "message": f"履约中心接管异常: {fc_e}",
                "data": None,
            }
