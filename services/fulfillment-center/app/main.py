...
from .inventory import _items_from_cpa, import_cpa, list_inventory, manual_update_status, normalize_account, summary
from .settings_store import get_all_settings, update_settings


app = FastAPI(title="Xianyu Codex Fulfillment", version="1.0.0")
...
def _cockpit_lines_from_payload(payload: Any) -> list[str]:
    ...
    for raw in _items_from_cpa(payload):
        ...
        lines.append(f"{json.dumps(item['email'], ensure_ascii=False)} {json.dumps(item['password'], ensure_ascii=False)} {json.dumps(extra, ensure_ascii=False)}")
    return lines
...
def health() -> dict[str, Any]:
    return {"ok": True, "service": "xianyu-codex-fulfillment"}
...
async def import_inventory(body: ImportCpaRequest) -> dict[str, Any]:
    ...
    if body.sync_cockpit:
        lines = _cockpit_lines_from_payload(body.payload)
...
async def import_inventory_files(
    ...
    result = import_cpa(payloads) if payloads else {"created": 0, "skipped": 0, "total": 0}
